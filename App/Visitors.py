...
import win32api, os, sqlite3, widgets, helper

DB_FILE = "Sukkur25L1.sqlite" # Name your db filename
...
def submit_form():
    ...
    try:
        ...
        if editing_last_entry_id is not None:
            ...
            cursor.execute(f"""
                UPDATE registrants 
                SET name=?, industry=?, occupation=?, phone=?, email=?, city=?, date_selected=?
                WHERE id=?
            """, (name, industry, occupation, phone, email, city, date_selected, editing_last_entry_id))
            editing_last_entry_id = None  # reset after successful update

        else:
            # Insert new record (normal new entry)
            cursor.execute(f"""
                INSERT INTO registrants (name, industry, occupation, phone, email, city, date_selected)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (name, industry, occupation, phone, email, city, date_selected))
        ...
        # Badge generation
        data = {"name": name, "industry": industry, "occupation": occupation}
...
def load_previous():
    ...
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, industry, occupation, phone, email, city FROM registrants ORDER BY id DESC LIMIT 1")
    ...
    if result:
        ...
        entry_industry.delete(0, tk.END); entry_industry.set(result[2])
...
def clear_form():
    global editing_last_entry_id
    for entry in [entry_name, entry_industry, entry_occupation, entry_phone, entry_email, entry_city]:
...
# GUI Setup
root = tk.Tk()
root.title("Sukkur 2025 Registration Management System")
root.geometry("1000x650")
root.resizable(True, True)

# === Background Image ===
background_image = Image.open("Resources/bg.jpg")  # Put your background.png in the same folder or give complete path 
...
logo_image = Image.open("Resources/logo.png")  # Replace with your actual logo file name
...
# Grid layout
labels = ["Name*", "Industry", "Occupation", "Phone Number*", "Email", "City"]
entries = []
entry_vars = []
...
for i, label in enumerate(labels):
    ...
    if label == "Industry":
        # create the custom searchable combobox (frame with entry)
        entry = widgets.SearchableComboBox(main_frame, INDUSTRY_OPTIONS, width=28, font=entry_font)
...
entry_name, entry_industry, entry_occupation, entry_phone, entry_email, entry_city = entries
